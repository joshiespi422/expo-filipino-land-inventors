        ...
        {schedule.map((item) => {
          ...
          return (
            <View key={item.id} className="mb-3">
              ...
              {/* BREAKDOWN SECTION WITH DYNAMIC COLORS */}
              {isExpanded && (
                <View
                  className={`p-4 rounded-b-2xl border-x border-b shadow-sm ${
                    isSelected
                ...
                >
                  <BreakdownRow label="Principal Due" value={item.principal} />
                  <BreakdownRow label="Interest Due" value={item.interest} />
                  <BreakdownRow label="Penalties Due" value={item.penalties} />
                  <BreakdownRow label="Late Fees Due" value={item.lateFees} />
                </View>
              )}
            </View>
          );
        ...
        >
          {isLoading ? (
            ...
            <Text className="text-white font-bold text-lg">
              {selectedIds.length > 0
                ? `Pay ₱${totalToPay.toLocaleString(undefined, {
